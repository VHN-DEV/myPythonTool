...
if [ -f "$SCRIPT_PATH/../composer.lock" ]; then
  (cd $SCRIPT_PATH/../ && COMPOSER_PROCESS_TIMEOUT=0 $PHP $PHPCOPTS composer.phar $DEVMODE update  -o -a --no-interaction --ignore-platform-reqs) ## -vvv --ignore-platform-reqs --prefer-source --no-interaction
else
  (cd $SCRIPT_PATH/../ && COMPOSER_PROCESS_TIMEOUT=0 $PHP $PHPCOPTS composer.phar $DEVMODE install -o -a --no-interaction --no-cache --ignore-platform-reqs) ## -vvv --ignore-platform-reqs --prefer-source --no-interaction
fi
